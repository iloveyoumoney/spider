...
class Spider():
    ...
    def getPageNum(self):
        ...
        if int(items.group(2)) >= 1:
            print ('\n这个主题共有图片', items.group(2), '页')
        else:
            print ('\n哎呀，木有您想要的图呢。。。')
    ...
    def saveImage(self,detailURL,name):
        fileName=name
        string='F:\Desktop\pexels\%s\%s' % (self.path, fileName)
    ...
    def makeDir(self, path):
        ...
        E=os.path.exists(os.path.join('F:\Desktop\pexels', self.path))
        if not E:
            os.makedirs(os.path.join('F:\Desktop\pexels',self.path))
            #os.chdir(os.path.join('F:\Desktop\pexels',self.path))
