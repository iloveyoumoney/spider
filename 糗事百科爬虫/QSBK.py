...
class Spider(object):
    ...
    def getDetailPage(self,detailURL):
        ...
        for item in items:
            print ('')
            print (number,'楼','\n楼主：',self.tool.replace(item[0]),'',self.tool.replace(item[1]),'岁','\n发言:',self.tool.replace(item[2]),'\n好笑：',item[3],'\n评论：',item[4],'\n赞：',item[5],'\n踩：',item[6])
            time.sleep(0.1)
            number+=1
        return items
    ...
    def getAllPage(self,start,end):
        if start==1:
            ...
            for page in range(2, end+1):
                print ('正在获取第', number, u'页的数据...')
                #detailURL = self.siteURL + '8hr/page/' + str(page) + '/?s=4964625'
                detailURL = self.siteURL + '8hr/page/' + str(page)
